...
function initGoogleMap() {
    ...
    if (!mapDiv) {
        console.error("googleMap div not found");
        return;
    }
...
async function startCamera() {
    ...
    console.error("cameraVideo not found");
}

if (dashVideo !== null) {
    dashVideo.srcObject = stream;
} else {
    console.warn("dashVideo not found (dashboard preview disabled)");
}
...
async function generateRoute() {
    ...
    speak("Generating route...");

    const origin = { lat: 30.3165, lng: 78.0322 };
    ...
    try {
        ...
        directionsService.route({
            ...
            if (status === "OK") {
                ...
                addLog(`🗺️ Route: ${leg.distance.text}`);

                // show status
                const statusBox = document.getElementById("routeStatus");
                if (statusBox) {
                    statusBox.innerText = `${leg.distance.text} • ${leg.duration.text}`;
                }
                ...
                console.error("Google route failed:", status);
                speak("Route failed");
            }
        });
...
async function takeCommand(command) {
 ...
 // ================= GOOGLE COORDINATE ROUTE =================
if (command.includes("route to")) {

    let destination = command.replace("route to", "give shortest path ").trim();

    if (!window.google || !directionsService) {
        speak("Map not ready yet");
        return;
    }

    directionsService.route({
        origin: { lat: 30.3165, lng: 78.0322},
        destination: destination, // 🔥 works for BOTH text + coords
        travelMode: "DRIVING"
    }, (result, status) => {
